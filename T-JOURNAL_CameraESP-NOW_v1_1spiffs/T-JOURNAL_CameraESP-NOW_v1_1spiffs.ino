...
// and start to send it immediatly after (it take 4 for VGA...etc).
//
...
// The screen of the board will show the MAC address that you must insert in the receiver sketch.
// Don't forget to change the Wifi SSID according your access point!
*/



// Insert your access point SSID
constexpr char WIFI_SSID[] = "THE_WIFI_NETWORK_NAME_YOUR_RECEIVER_WILL_CONNECT_ON";// set here the SSID of your router AP (from internet box, smartphone...)
...
// Pin definition for TTGO_T-JOURNAL
...
void setup() {
  ...
  display.print("EspNow LongRange Cam!");
  display.display();
...
void initCamera()
{
  ...
  if (psramFound()) {
    config.frame_size = FRAMESIZE_VGA; //FRAMESIZE_UXGA; // FRAMESIZE_ + QVGA|CIF|VGA|SVGA|XGA|SXGA|UXGA //FRAMESIZE_QVGA  /!\ADJUST displayed values (2) below!
    ...
    display.print("Framesize VGA");//CHANGE WITH SETTINGS
    display.display();
  } else {
