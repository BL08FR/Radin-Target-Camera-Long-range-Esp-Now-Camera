...
Serial.println(WiFi.channel());
  ...
  server.on("/img", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/savedpic.jpg", "image/jpeg", false);//false parameter added for test
  });
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len) {
  ...
  switch (*data++)
  {
    ...
    case 0x02:
      ...
      File file = SPIFFS.open("/picture.jpg",FILE_APPEND);
      if (!file)
      ...
      if (currentTransmitCurrentPosition == currentTransmitTotalPackages)
      {
        ...
        File file = SPIFFS.open("/picture.jpg");
        Serial.println(file.size());
        file.close();
        SPIFFS.remove("/savedpic.jpg");//delete last saved picture (savedpic.jpg)
        SPIFFS.rename("/picture.jpg", "/savedpic.jpg");//rename temporary file (picture) into saved picture(savedpic.jpg)
